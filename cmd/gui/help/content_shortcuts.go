...
var contentShortCutsRef = content{
	...
	table{
		...
		rows: []tableRow{
			...
			{{codeCopyable("export-image:metadata")}, {
			...
			{{codeCopyable("heat-map")}, {"Turns on the heat map instrument (defaults to ", code(`Activity`), ")"}},
			{{codeCopyable("heat-map-colors:colors")}, {"Sets the colors used for heat map images - where ", italic("colors"), " is a semi-colon delimited list of colors (as HTML hex color, ", code(`#rrggbb`), ")\n",
				italic("The list must contain at least two valid colors - anything else resets heat map colors.")}},
			{{codeCopyable("heat-map:type")}, {"Turns on the heat map instrument with ", italic("type"), " (", code(`Activity`), "|", code(`Age`), "|", code(`Births`), "|", code(`Freshness`), "|", code(`Longevity`), "|", code(`Occupancy`), "|", code(`Phase Parity`), "|", code(`All`), ")\n", italic("(other values will turn heat mapper off)")}},
			{{codeCopyable("heat-map-reveal")}, {"Reveals (shows) the current heat map"}},
			{{codeCopyable("heat-map-save")}, {
				"Saves the current heat map as PNG",
				"\n", italic("Note: If "), code("heat-map:type"), italic(" is set to "), code("All"), italic(" then images for each heat map type are created. No images are created when heat mapping is off."),
			}},
			{{codeCopyable("heat-map-save:metadata")}, {
			...
			{{codeCopyable("record-animation-save")}, {"Saves the current recording as an animation ", italic("(only works when recording is on - e.g. "), codeCopyable(`record:1`), italic(")")}},
			...
			{{codeCopyable("step-back-by:n")}, {"Performs a step back by ", italic("n"), " (>0) ", italic("(only works when recording is on - e.g. "), codeCopyable(`record:1`), italic(")")}},
