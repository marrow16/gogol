...
func TestAgeHeatMapInstrument_Step(t *testing.T) {
	...
	g.SetCell(2, 1, true)
	g.SetCell(2, 2, true)
	i := NewAgeHeatMapInstrument(g)
	assert.Equal(t, AgeHeatMapper, i.Type())
	for range 100 {
...
func TestAgeHeatMapInstrument_StepAhead(t *testing.T) {
	g, err := NewGrid(5, 10, StandardRule, WrapAll, DeadBoundary)
	require.NoError(t, err)
	g.SetCell(0, 2, true)
	g.SetCell(1, 0, true)
